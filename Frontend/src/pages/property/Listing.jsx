  ...
  ? Object.entries(localityareas).reduce((areas, [locality, areaList]) => {
      if (locality.toLowerCase().startsWith(searchTerm.toLowerCase())) {
        ...
        return [
          ...areas,
          ...areaList
            .filter((area) => area.toLowerCase().startsWith(searchTerm.toLowerCase()))
            .map(area => ({
              name: area,
...
const filteredResults = [...filteredLocalities, ...filteredAreas];
    ...
    <>
      ...
      >
        ...
        <div className="px-3 flex flex-col gap-8 py-6 sticky top-0 z-20 bg-black">
          ...
          <div className="flex justify-between gap-14 w-full flex-wrap rounded-md">
            <div className="flex items-center justify-between  flex-col md:flex-row lg:flex-row">
              <div className="bg-white h-14 w-64 md:w-fit lg:w-fit flex items-center justify-between text-black px-4">
  ...
  <input
    ...
    onChange={(e) => handleSearchChange(e.target.value)}
    placeholder="Search localities or areas"
    className="w-70 py-1 px-2 border border-gray-300 rounded-lg"
  />
  {showSuggestions && filteredResults.length > 0 && (
  <div className="relative">
    <div className="z-50 absolute bg-white shadow-lg rounded-lg text-center w-64 top-[25px]">
      {filteredResults.map((result, index) => (
        ...
        >
          ...
          <span className="text-sm text-gray-500 text-right">{result.type}</span>
        </p>
      ))}
    </div>
  </div>
